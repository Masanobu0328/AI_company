...
def strip_emoji(text):
    ...
    for char in text:
        ...
        cp = ord(char)
        # 絵文字・記号ブロックを除去
        if cp in range(0x1F600, 0x1F700) or cp in range(0x1F300, 0x1F600) or \
           cp in range(0x1F700, 0x1F900) or cp in range(0x1F900, 0x1FA00) or \
           cp in range(0x2600, 0x2700) or cp in range(0xFE00, 0xFE0F) or \
           cp in range(0x1FA00, 0x1FB00) or cp == 0x200D:
            continue
        result.append(char)
...
async def compress_memory_if_needed(agent_key):
    """メモリが20エントリを超えたら古い部分をClaudeで圧縮する"""
    key_to_folder = {"secretary": "secretary", "sales": "sales", "pm": "pm",
                     "dev": "development", "development": "development", "marketing": "marketing"}
    folder = key_to_folder.get(agent_key, agent_key)
    path = f"agents/{folder}/memory.md"
    ...
    try:
        response = claude.messages.create(
            ...
            max_tokens=400,
            messages=[{"role": "user", "content": f"""以下のエージェントの記憶ログを、重要な決定・方針・合意事項だけを残して箇条書きで圧縮してください。
日付は省略し、「- 内容」の形式で5〜10行にまとめてください。
...
"""
    ...
    for key, folder in key_to_folder.items():
        ...
        secretary_rule = """
## 秘書の特別ルール
- あなたは会社全体の状況を把握する唯一のエージェント
- 各エージェントからの報告はあなたのメモリに自動蓄積される
- CEOから「進捗は？」「誰が何してる？」と聞かれたら、メモリの報告をもとに整理して答えること
- 自分が確認・実行できないことは「できます」「しました」と絶対に言わない
- できることとできないことを明確に区別して答えること
- 「学習しました」「記憶しました」「保存しました」は絶対に言わない
""" if key == "secretary" else ""
        ...
        tone_rule = "敬語を使い、丁寧に話すこと。" if key == "secretary" else "敬語は不要。カジュアルに話すこと。「〜です」「〜ます」は使わず、「〜だ」「〜だね」「〜しよう」のような口調で話す。"
...
## 応答スタイルの絶対ルール
- {tone_rule}
- 絵文字・デコレーション（━━━など）は一切使わない
- 返答は最大3文まで。それ以上は書かない
- 「了解しました」で終わらない。必ず自分の意見・提案・質問・次のアクションを加える
- 箇条書きは最大3項目まで。それ以上は要約する
- 前置き・まとめ・「以上です」などの締めは不要

## システム構成（重要）
- あなたはDiscord Botシステムの一部として動作している
- GitHubへの保存・ログ記録はシステムが自動で行う
- 「学習しました」「記憶しました」「保存しました」は絶対に言わない
- 事業方針・決定事項・重要な合意が出たら返答末尾に [SAVE: 一行で要約] を追加すること（CEOには表示されない）

## 成果物の出力方法
コード・設計書・提案書・HTMLなどは以下の形式で出力すること（CEOには表示されず自動保存）：
[FILE:projects/goldcoast/[担当フォルダ]/ファイル名.拡張子]
内容
[/FILE]
- 営業リスト・一覧データは .xlsx 形式で出力。CSV形式（1行目=ヘッダー）で記述すること
...
- できることとできないことを明確に区別して答えること
- 「学習しました」「記憶しました」「保存しました」は絶対に言わない
""" if agent_key == "secretary" else ""
    tone_rule = "敬語を使い、丁寧に話すこと。" if agent_key == "secretary" else "敬語は不要。カジュアルに話すこと。「〜です」「〜ます」は使わず、「〜だ」「〜だね」「〜しよう」のような口調で話す。"
...
def save_excel_locally(path, content, timestamp):
    """CSV形式のコンテンツをExcelファイルとしてローカル保存"""
    local_path = os.path.join(BASE_DIR, path.replace("/", os.sep))
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
...
async def save_file_outputs(reply, timestamp):
    """[FILE:path]内容[/FILE] ブロックを検出して保存（.xlsxはローカルExcel、それ以外はGitHub）"""
    pattern = r'\[FILE:([^\]]+)\]\n?(.*?)\[/FILE\]'
...
async def handle_agent_messaging(guild, sender_key, reply):
    ...
    for target_key, content in msg_matches:
        ...
        if target_key == "secretary":
            ...
            entry = f"[{sender_name}より報告] {content}"
            await append_agent_memory("secretary", entry, timestamp)
            update_agent_prompt("secretary")
            print(f"秘書メモリ更新: {sender_name} → {content[:50]}")
            continue

        # エージェント間の会話を最大3ラウンド続ける
        async with agent_chat.typing():
            try:
                messages = [{"role": "user", "content": f"{sender_name}から：{content}"}]
                sender_agent = AGENTS.get(sender_key)
                decisions = []

                for round_num in range(3):
                    current_speaker = target_agent if round_num % 2 == 0 else sender_agent
                    current_key = target_key if round_num % 2 == 0 else sender_key

                    response = claude.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=200,
                        system=current_speaker["prompt"],
                        messages=messages,
                    )
                    raw_reply = response.content[0].text

                    # [SAVE:]を抽出して記憶
                    save_matches = re.findall(r'\[SAVE:\s*(.+?)\]', raw_reply, re.DOTALL)
                    decisions.extend([s.strip() for s in save_matches])

                    agent_reply = re.sub(r'\[.+?\]', '', raw_reply, flags=re.DOTALL).strip()

                    if WEBHOOK_AGENT_CHAT:
                        await webhook_send(WEBHOOK_AGENT_CHAT, current_key, agent_reply)

                    messages.append({"role": "assistant", "content": agent_reply})

                    # 最後のラウンド以外は続きの発言を促す
                    if round_num < 2:
                        next_name = sender_agent["name"] if round_num % 2 == 0 else target_agent["name"]
                        messages.append({"role": "user", "content": f"（{next_name}として返答してください）"})

                # 会話全体をClaudeで要約して両エージェントのメモリに保存
                ts = datetime.utcnow()
                conversation_text = "\n".join(
                    f"{m['role']}: {m['content']}" for m in messages if m['content'] and "として返答" not in m['content']
                )
                try:
                    summary_response = claude.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=150,
                        messages=[{"role": "user", "content": f"以下のエージェント間の会話を1〜3行で要約してください。決定事項・合意事項・次のアクションを中心に。\n\n{conversation_text}"}],
                    )
                    summary = summary_response.content[0].text.strip()
                except Exception:
                    summary = f"{sender_name}と{target_name}の会話：{content[:80]}"
                ...
                await append_agent_memory("secretary", f"[{sender_name}×{target_name}] {summary}", ts)
...
def route_message_to_agents(content):
    ...
    routing_hints = {
        "sales":     ["営業", "グラント", "セールス", "売上", "クライアント", "リード", "提案書", "商談", "受注"],
        "marketing": ["マーケ", "森岡", "SNS", "ブランド", "広告", "戦略", "集客", "投稿", "コンテンツ", "Threads"],
        "dev":       ["開発", "ウォズ", "コード", "実装", "バグ", "システム", "技術", "プログラム", "エラー", "LP"],
        "pm":        ["PM", "グレン", "プロジェクト", "タスク", "スケジュール", "進捗", "管理", "整理", "優先"],
        "secretary": ["秘書", "菅", "日程", "調整", "連絡", "会議", "全体", "まとめ", "確認"],
    }
    matched = [key for key, kws in routing_hints.items() if any(kw in content for kw in kws)]
    return matched if matched else ["secretary"]


async def handle_all_hands(message, content):
    """全体チャンネル: 担当エージェントのみ応答、返答内容は秘書メモリに自動記録"""
    ...
    for agent_key in primary_agents:
        ...
        async with message.channel.typing():
            try:
                ...
                # [SAVE:]を抽出して当該エージェントのメモリに保存
                save_matches = re.findall(r'\[SAVE:\s*(.+?)\]', raw_reply, re.DOTALL)
                for s in save_matches:
    ...
    if is_mention or is_agent_channel:
            ...
            async with message.channel.typing():
                ...
                if is_all_hands_channel(channel_name):
                    ...
                    if targeted_key:
                        ...
                        msg = f" **{target_name}の個人メモリに保存しました**" if success else " 保存に失敗しました。"
                    else:
                        success = await append_shared_knowledge(content, message.created_at)
                        msg = " **共有ナレッジに保存しました**（全エージェントが参照）" if success else " 保存に失敗しました。"
                    ...
                    if success:
                        ...
                        await message.channel.send(f" 保存に失敗しました。")
                # エージェントとしても応答
                try:
                    response = claude.messages.create(
                        model="claude-haiku-4-5-20251001",
        ...
        async with message.channel.typing():
            try:
                ...
                # モデルに応じてシステムプロンプトの文字数制約を切り替える
                length_rule = (
                    "返答の長さは自由。成果物・分析・リストなど必要な分だけ詳しく書くこと。"
                    if is_skill else
                    "返答は5行以内に収めること。会話・確認・簡単な質問は2行以内。"
                )
